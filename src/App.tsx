...
import React, { 
  ...
  ReactNode, 
  SetStateAction, 
  ReactElement, // 'icon' の型
  HTMLAttributes,
  ButtonHTMLAttributes,
  InputHTMLAttributes,
  TextareaHTMLAttributes,
  Dispatch, // SetStateAction の型
  JSXElementConstructor, // cloneElement ため
  useRef,
  useEffect
} from 'react';
...
/* ▼▼▼ [修正] w-full を追加し、タブ切り替えで幅が変わらないようにする ▼▼▼ */
...
/* ▼▼▼ [修正] 根本的なスクロール問題の解消。flex-1 で高さを確保しつつ、overflow-auto で中身をスクロールさせる ▼▼▼ */
...
interface Post {
  id: string;
  authorId: string;
  recipient: string | string[]; // ▼▼▼ [修正] 宛先を string[] も許容
  text: string;
  ts: number;
  reactions: PostReaction[];
  replied?: boolean; // INBOX_POSTS にのみ存在
}

const KNOWLEDGE: KnowledgeFile[] = [
  { id:1, type:'PDF', title:'2025年冬期 新商品マニュアル', uploaded:'2025-10-28', size:'2.5MB', important:true, folderId: 'f1' },
  { id:2, type:'PPTX', title:'秋キャンペーン共有会.pptx', uploaded:'2025-10-27', size:'8.2MB', important:true, folderId: 'f1' },
  { id:3, type:'PDF', title:'衛生チェックリスト改定（店舗）', uploaded:'2025-10-25', size:'0.9MB', important:false, folderId: 'f2' },
  { id:4, type:'PDF', title:'VMD週次アップデート_1025.pdf', uploaded:'2025-10-25', size:'5.1MB', important:false, folderId: 'f2' },
  { id:5, type:'PDF', title:'新人研修ガイドv3', uploaded:'2025-10-22', size:'1.2MB', important:false, folderId: 'f3' },
  { id:6, type:'PDF', title:'価格改定ポリシー（確定）', uploaded:'2025-10-19', size:'0.6MB', important:true, folderId: 'f2' },
];
const KNOWLEDGE_FOLDERS: KnowledgeFolder[] = [
  { id: 'f1', name: '新商品・キャンペーン資料', updated: '2025-10-28' },
  { id: 'f2', name: '店舗運営マニュアル', updated: '2025-10-25' },
  { id: 'f3', name: '研修・オンボーディング', updated: '2025-10-22' },
];
const INITIAL_NOTICES: Notice[] = [ { id:'n1', title:'今週の重点接客ポイント', body:'テスター声かけは「お肌悩み→提案→拭き取り」。各自3回は声掛け。', created:'2025-10-29 09:00' } ];

const USER: User = { id: 'you', name: '田中（店長）', avatar: 'https://placehold.co/40x40/B0B0B0/FFFFFF?text=店長' };

const MEMBERS: Member[] = [ 
  { id:'you', name:'田中（店長）' }, // ログインユーザー（店長）
  { id:'sato', name:'佐藤' }, 
  { id:'suzuki', name:'鈴木' }, 
...
const MY_POSTS: Post[] = [
  { id:'m1', authorId:'you', recipient:'sato', text:'VMD棚替えの対応ありがとう！すごく良くなった。', ts: Date.parse('2025-10-28T11:00:00+09:00'), reactions: [{emoji: '👍', count: 1, by: ['sato']}] },
  { id:'m2', authorId:'you', recipient:'zen', text:'月初ミーティングのアジェンダ共有', ts: Date.parse('2025-10-27T19:00:00+09:00'), reactions: [] },
  { id:'m3', authorId:'you', recipient:'suzuki', text:'昨日のフォロー助かった、ありがとう！', ts: Date.parse('2025-10-29T11:10:00+09:00'), reactions: [{emoji: '😊', count: 1, by: ['suzuki']}]},
  { id:'m4', authorId:'you', recipient:'kimura', text:'シフトの件、確認しました。', ts: Date.parse('2025-10-26T18:00:00+09:00'), reactions: [] },
];
const INBOX_POSTS: Post[] = [
  { id:'i1', authorId:'suzuki', recipient:'you', text:'接客サポート助かりました！', ts: Date.parse('2025-10-27T09:00:00+09:00'), replied: true, reactions: [{emoji: '👍', count: 1, by: ['you']}] },
  { id:'i2', authorId:'sato', recipient:'you', text:'ギフト包装の件、確認お願いします', ts: Date.parse('2025-10-28T19:20:00+09:00'), replied:false, reactions: [] },
  { id:'i3', authorId:'kimura', recipient:'you', text:'シフト代行感謝です。', ts: Date.parse('2025-10-26T18:15:00+09:00'), replied: true, reactions: [] },
  { id:'i4', authorId:'yamaguchi', recipient:'you', text:'明日の入り時間大丈夫そうですか？', ts: Date.parse('2025-10-27T14:00:00+09:00'), replied:false, reactions: [] },
];
const ALL_PUBLIC_POSTS: Post[] = [
  { id:'e1', authorId:'sato', recipient:'zen', text:'テスター補充導線を改善しました', ts: Date.parse('2025-10-29T12:00:00+09:00'), reactions: [{emoji: '💡', count: 2, by: ['you', 'kimura']}] },
  { id:'e2', authorId:'suzuki', recipient:'sato', text:'VMD棚替えの対応ありがとう！', ts: Date.parse('2025-10-28T10:30:00+09:00'), reactions: [{emoji: '😊', count: 1, by: ['sato']}] },
  { id:'e3', authorId:'kimura', recipient:'yamaguchi', text:'ストック整理の件、了解です。', ts: Date.parse('2025-10-27T20:00:00+09:00'), reactions: [] },
  { id:'e4', authorId:'sato', recipient:'suzuki', text:'フォローありがとう！', ts: Date.parse('2025-10-29T11:15:00+09:00'), reactions: [{emoji: '👍', count: 1, by: ['suzuki']}] },
];
...
// --- シフトデータ (日付string: [メンバーID, 時間string]) ---
type ShiftData = Record<string, { m: string; t: string }[]>;
const SHIFTS: ShiftData = {
  "2025-11-03": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-15:00' }, { m: 'suzuki', t: '休み' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2025-11-04": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '休み' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2025-11-05": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '休み' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2025-11-06": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '休み' } ],
  "2025-11-07": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '休み' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
  ...
  "2025-11-10": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-15:00' }, { m: 'suzuki', t: '休み' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2025-11-11": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '休み' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2025-11-12": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '休み' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2025-11-13": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '休み' } ],
  "2025-11-14": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '休み' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
  ...
  "2025-11-17": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-15:00' }, { m: 'suzuki', t: '休み' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2025-11-18": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '休み' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2025-11-19": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '休み' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2025-11-20": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '休み' } ],
  "2025-11-21": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '休み' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
  ...
  "2025-11-24": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-15:00' }, { m: 'suzuki', t: '休み' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2025-11-25": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '休み' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2iOS-11-26": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '休み' }, { m: 'kimura', t: '12:00-21:00' } ],
  "2025-11-27": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '9:00-18:00' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '休み' } ],
  "2025-11-28": [ { m: 'you', t: '9:00-18:00' }, { m: 'sato', t: '9:00-18:00' }, { m: 'suzuki', t: '休み' }, { m: 'yamaguchi', t: '11:00-20:00' }, { m: 'kimura', t: '12:00-21:00' } ],
...
const ALL_DAY_EVENTS: AllDayEvent[] = [
  { id: 'e1', title: '新商品A 発売日', start: '2025-11-05', end: '2025-11-05', color: 'bg-green-100 text-green-800' },
  { id: 'e2', title: '週末WポイントCP', start: '2025-11-07', end: '2025-11-09', color: 'bg-indigo-100 text-indigo-800' },
  { id: 'e3', title: '週末WポイントCP', start: '2025-11-14', end: '2025-11-16', color: 'bg-indigo-100 text-indigo-800' },
  { id: 'e4', title: 'BLACK FRIDAY セール', start: '2025-11-28', end: '2025-11-30', color: 'bg-gray-800 text-white' },
];
...
// ▼▼▼ [修正] JST基準で日付を取得するように修正 (toISOString依存を削除) ▼▼▼
...
const getWeekStart = (date: Date) => {
  ...
  d.setHours(0, 0, 0, 0); // JSTの0時0分0秒にリセット
  const day = d.getDay(); // 0 (Sun) - 6 (Sat)
  const diff = d.getDate() - day + (day === 0 ? -6 : 1); // day=0(Sun)なら-6、day=1(Mon)なら+0
...
const parseShiftTime = (time: string) => {
  ...
  if (parts.length !== 2) {
    return { startHour: null, endHour: null, isOff: false }; // 不明な形式
  }
...
// ▼▼▼ [修正] この関数は使用されなくなりましたが、VoiceTrustV8Style のために残しておきます ▼▼▼
const getShiftForMember = (date: Date, memberId: string) => {
  ...
  const time = shift ? shift.t : '休み'; // データがなければ休み
...
export default function TsumugiApp(){
  ...
  const tabs: [string, string, ReactElement<React.SVGProps<SVGSVGElement>>][] = role==='manager'
    /* ▼▼▼ [修正] 「ダッシュボード」を「ホーム」の隣に移動 ▼▼▼ */
    ? [['home','ホーム',<HomeIcon/>], ['dashboard','ダッシュボード',<DashIcon/>], ['calendar', 'カレンダー', <CalendarIcon/>], ['voice','ひとこと',<ChatIcon/>],['knowledge','ナレッジ',<BookIcon/>],['integrations','連携',<SparklesIcon/>]]
    /* ▲▲▲ 修正 ▲▲▲ */
    : [['home','ホーム',<HomeIcon/>], ['calendar', 'カレンダー', <CalendarIcon/>], ['voice','ひとこと',<ChatIcon/>],['knowledge','ナレッジ',<BookIcon/>]];

  const headerClass = role === 'manager'
    ? "bg-gray-800 text-white" // 管理者用の色
    : "bg-gradient-to-r from-indigo-600 to-purple-600 text-white"; // スタッフ用の色

  return (
    /* ▼▼▼ [根本修正] min-h-screen を h-screen overflow-hidden に変更 ▼▼▼ */
    /* これによりページ全体がスクロールしなくなり、内部の sticky と flex-1 が正しく機能する */
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            <div className={`absolute right-0 mt-1 bg-white border rounded-xl shadow-lg w-48 p-2 text-gray-900 ${menuOpen ? 'block' : 'hidden'}`}>
              <div className="p-2 text-xs text-gray-500">スタッフ一覧</div>
            ...
            <div className="p-2 text-sm font-medium">{GET_NAME('sato')}</div>
              ...
              <button onClick={()=>{ setRole(r=>r==='manager'?'staff':'manager'); setMenuOpen(false); setTab('home'); }} className="w-full text-left p-2 text-sm rounded-lg hover:bg-gray-100">
                {role==='manager'?'スタッフ表示に切替':'管理表示に切替'}
              </button>
              <button onClick={() => setMenuOpen(false)} className="w-full text-left p-2 text-sm rounded-lg hover:bg-gray-100">ログアウト</button>
            </div>
          </div>
        </div>
      </div>
...
function HomeDashboard({ role }: HomeDashboardProps){
  ...
  const unreadTasks: UnreadTask[] = [
    { id: 't1', type: 'knowledge', title: '秋キャンペーン共有会.pptx', due: '本日中', from: '本部' },
    { id: 't2', type: 'notice', title: '今週の重点接客ポイント', due: '本日中', from: GET_NAME('you') }, // 'tencho' -> 'you' に修正
  ];
 
  const thanks: Thank[] = [
    { id: 'th1', from: 'sato', to: 'yamaguchi', text: '昨日のレジ締め代行、感謝です！' },
    { id: 'th2', from: 'you', to: 'kimura', text: 'VMD変更、すごく良くなった！' },
    { id: 'th3', from: 'sato', to: 'you', text: 'フォローありがとう！' }, 
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="font-semibold text-lg text-gray-900">あなた宛ての確認事項（{unreadTasks.length}件）</div>
            </div>
          </CardHeader>
            ...
            {unreadTasks.map(task => (
              <div key={task.id} className="flex items-center gap-3 p-3 border rounded-xl hover:bg-gray-50 cursor-pointer">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-500">From: {task.from} / 期限: {task.due}</div>
                </div>
                <Button variant="outline" size="sm">今すぐ確認</Button>
              </div>
            ))}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <div className="font-semibold text-lg text-gray-900">今日の必須ナレッジ（AI要約）</div>
          </CardHeader>
          <CardContent>
            <div className="p-4 bg-purple-50 rounded-xl border border-purple-200">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-purple-600"><StarIcon className="w-5 h-5"/></span>
                <div className="font-semibold text-purple-800">2025年冬期 新商品マニュアル</div>
              </div>
              <div className="text-sm text-gray-700 space-y-1">
                <p>1. <span className="font-bold">重要ポイント:</span> 新保湿成分「リピジュアEX」配合が最大の売りです。</p>
                <p>2. <span className="font-bold">現場アクション:</span> 敏感肌のお客様には、まずテスターでの声かけを徹底してください。</p>
                <p>3. <span className="font-bold">注意事項:</span> 既存の「モイストローション」とは併用せず、切り替えを推奨します。</p>
              </div>
              <Button size="sm" className="mt-3">ファイルを開く</Button>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <div className="font-semibold text-lg text-rose-700">最近の「ありがとう」👏</div>
          </CardHeader>
          <CardContent className="space-y-3">
            {thanks.map(t => (
            ...
            <Button variant="outline" className="w-full">ありがとうを贈る</Button>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <div className="font-semibold text-lg text-gray-900">店長からのお知らせ</div>
          </CardHeader>
          <CardContent className="space-y-3">
            {notices.map(n => (
...
// =================== CalendarView (GCal風 UI) ===================

// --- Main Component ---
function CalendarView() {
  const [viewMode, setViewMode] = useState<'month' | 'week' | 'day'>('week');
  const [currentDate, setCurrentDate] = useState(new Date('2025-11-03T12:00:00+09:00')); // 11/3 (月) を基準
  const today = useMemo(() => new Date('2025-11-03T12:00:00+09:00'), []); // 固定の今日
  ...
  return (
    <div className="flex flex-col h-full min-h-[70vh]"> {/* カレンダー全体が高さを確保するように */}
      <Card className="flex flex-col flex-1 min-h-0"> {/* カードも追従 */}
        {/* --- Header --- */}
        <CardHeader className="pb-0">
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center gap-1 rounded-lg p-0.5">
              <ViewButton value="month" label="月" />
              <ViewButton value="week" label="週" />
              <ViewButton value="day" label="日" />
            </div>
          </div>
        </CardHeader>
       
        {/* --- Content --- */}
        {/* flex-1 min-h-0 で中身がスクロール可能に */}
...
// --- WeekViewGcal (Googleカレンダー風タイムグリッド・週) ---
function WeekViewGcal({ currentDate, today }: { currentDate: Date, today: Date }) {
  ...
  const weekEvents = ALL_DAY_EVENTS.filter(e => {
    ...
    weekEnd.setHours(23, 59, 59); // 週の終わり
  ...
  const allDayEventsWithRow = weekEvents.map(event => {
    ...
    if (colStart === -1 && start < weekDays[0]) colStart = 0; // 週をまたいで開始
   
    let colEnd = weekDays.findIndex(day => isSameDay(day, end));
    if (colEnd === -1 && end > weekDays[6]) colEnd = 6; // 週をまたいで終了
   
    if (colStart === -1 || colEnd === -1) return null; // 範囲外
   
    const duration = colEnd - colStart + 1;
    // TODO: 本来は行の重なりをチェックする
    const row = 1; 
  ...
  const allDayRowHeight = Math.max(1, maxAllDayRows) * 32 + 8; // 1行32px + padding
  ...
  return (
    <div className="h-full overflow-auto relative" style={{ minWidth: '800px' }}>
      ...
      >
        ...
        {/* --- Row 2: All-Day --- */}
        <div className="sticky top-[73px] z-20 bg-white border-b border-r text-right pr-2 pt-1 text-xs text-gray-400" style={{ gridColumn: 1, gridRow: 2, height: `${allDayRowHeight}px` }}>終日</div>
        <div 
        ...
        >
          {weekDays.map((day, dayIndex) => {
            const dateStr = toDateString(day); // <-- ここの`toDateString`が修正された
            const shifts = (SHIFTS[dateStr as keyof ShiftData] || [])
              .map(s => {
            ...
            const dayWidthPercent = 100; // 1日分の幅 (100%)
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    top: `${topOffset}px`,
                    height: `${height}px`,
                    left: `calc(${dayIndex * (100 / 7)}% + ${leftOffset * (100 / 7 / 100)}% + 2px)`, // (100/7) = 1日の幅
                    width: `calc(${widthPercent * (100 / 7 / 100)}% - 4px)`, // 隙間
                    opacity: 0.9,
                  }}
                  title={`${GET_NAME(s.memberId)}: ${s.time}`}
                >
                  <div className="text-[10px] font-semibold">{GET_NAME(s.memberId).split('（')[0]}</div>
...
function MonthView({ currentDate, today }: { currentDate: Date, today: Date }) {
  ...
  const monthStart = getWeekStart(d); // 月の第1週の月曜日
 
  const days = Array.from({ length: 42 }).map((_, i) => { // 6週 * 7日
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* --- Day Cells --- */}
      <div className="grid grid-cols-7 flex-1" style={{ gridTemplateRows: 'repeat(6, minmax(120px, 1fr))' }}> {/* 6行固定 */}
        {days.map((day, i) => {
          ...
          // ▼▼▼ [修正] 全メンバーのシフト情報（休み含む）を生成 ▼▼▼
          const shifts = MEMBERS.map(member => {
            const shiftData = dayShiftsData?.find(s => s.m === member.id);
            // データが無い日は、当月ならランダム、当月外なら「休み」
            const time = shiftData ? shiftData.t : (isCurrentMonth ? (Math.random() > 0.4 ? '9:00-18:00' : '休み') : '休み'); 
           
            let displayTime = '休';
            if (time !== '休み' && time.includes('-')) {
                const parts = time.split('-');
                if (parts.length === 2) {
...
// --- DayViewGcal (Googleカレンダー風タイムグリッド・日) ---
function DayViewGcal({ currentDate, today }: { currentDate: Date, today: Date }) {
  ...
  const allDayRowHeight = Math.max(1, allDayEvents.length) * 32 + 8; // 1行32px + padding
  ...
  return (
    <div className="h-full overflow-auto relative" style={{ minWidth: '300px' }}>
      ...
      >
        ...
        >
          <div className="text-xs text-gray-500">{['日', '月', '火', '水', '木', '金', '土'][currentDate.getDay()]}</div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  width: `calc(${widthPercent}% - 4px)`, // 隙間
                  opacity: 0.9,
                }}
                title={`${GET_NAME(s.memberId)}: ${s.time}`}
              >
...
function VoiceTrustV8Style({ role }: { role: string }){
  ...
  const [recipient, setRecipient] = useState<string[]>([]); // 宛先IDの配列
  ...
  const previewCount = 3; // プレビューで表示する件数
  ...
  /* ▼▼▼ [追加] エラー修正: handleCategoryChange を定義 ▼▼▼ */
    ...
    const updatePostList = (posts: Post[]): Post[] => {
      return posts.map(p => {
        ...
        if (existingReactionIndex > -1) {
          ...
          if (userIndex > -1) {
            // ユーザーが既にこの絵文字でリアクションしている -> リアクションを削除
            reaction.by.splice(userIndex, 1);
            reaction.count = reaction.by.length;
            if (reaction.count === 0) {
              // 誰もリアクションしていなければ、絵文字のエントリ自体を削除
              newReactions.splice(existingReactionIndex, 1);
            }
          } else {
            // ユーザーがこの絵文字でリアクションしていない -> ユーザーを追加
            reaction.by.push(loginUserId);
            reaction.count = reaction.by.length;
          }
        } else {
          // この投稿に対する新しい絵文字のリアクション
  ...
  /* ▼▼▼ [修正] リアクションバー コンポーネント (大幅機能追加) ▼▼▼ */
  const ReactionBar = ({ post }: { post: Post }) => {
    const [showPicker, setShowPicker] = useState(false);
    /* ▼▼▼ [追加] ピッカーの参照とカスタムスタンプ用の state ▼▼▼ */
    const pickerRef = useRef<HTMLDivElement>(null); // ピッカー全体の参照
    const [customEmojis, setCustomEmojis] = useState<string[]>(['🎉', '🚀']); // カスタムスタンプ
    const [showCustomInput, setShowCustomInput] = useState(false);
    const [customInput, setCustomInput] = useState('');
    /* ▲▲▲ 追加 ▲▲▲ */
  
    // Slack風の主要な絵文字
    /* ▼▼▼ [修正] 絵文字の種類を大幅に増やす (30種類) ▼▼▼ */
    const EMOJIS = [
      '👍', '😊', '❤️', '💡', '🤔', '👏', '🙏', '💯', '🔥', '😇', 
      '😂', '🥲', '🥳', '🤯', '😱', '😥', '😭', '😮', '👀', '✅', 
      '✨', '💪', '🍻', '🫡', '🤝', '✔️', '🌟', '🎊', '🧠', '✍️'
    ...
    const handleAddCustomEmoji = () => {
      ...
      // (簡易バリデーション: 1文字の絵文字かチェック - 絵文字の判定は複雑なためここでは文字長のみで代用)
      if (emoji && emoji.length > 0 && emoji.length <= 2) { 
        if (!EMOJIS.includes(emoji) && !customEmojis.includes(emoji)) {
          setCustomEmojis([emoji, ...customEmojis]);
        }
        setCustomInput('');
        setShowCustomInput(false);
        handleEmojiClick(emoji); // すぐにリアクションする
      } else {
        // (簡易エラーハンドリング)
        console.warn("Invalid emoji input");
        setCustomInput('');
      }
    };
    /* ▲▲▲ 追加 ▲▲▲ */
    ...
    return (
      /* ▼▼▼ [修正] ref をピッカーボタンとピッカー本体を囲む親要素に設定 ▼▼▼ */
      <div className="flex items-center gap-2 relative" ref={pickerRef}>
        {/* 集計されたリアクション */}
        {post.reactions.map(r => (
          <div key={r.emoji} className="group relative"> {/* ホバー用のラッパー */}
            <button
              onClick={() => handleEmojiClick(r.emoji)}
              /* ▼▼▼ [修正] title 属性を削除 (カスタムツールチップで代替) ▼▼▼ */
              className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs transition-all ${
                userReactions.has(r.emoji)
                ? 'bg-indigo-600 text-white' // 自分がリアクション済み
            ...
            {/* ▼▼▼ [追加] ホバー時のツールチップ ▼▼▼ */}
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 w-max max-w-xs bg-gray-800 text-white text-xs rounded-md px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20">
              {r.by.map(GET_NAME).join(', ')} がリアクションしました
            </div>
            {/* ▲▲▲ 追加 ▲▲▲ */}
          </div>
        ))}
        ...
        {/* 絵文字ピッカーボタン */}
        <button 
          onClick={() => { setShowPicker(!showPicker); setShowCustomInput(false); }}
          className="p-1 rounded-full text-gray-500 hover:bg-gray-100 hover:text-gray-700"
        >
          {/* Slack風のスタンプ追加アイコン（ChatIconで代用） */}
          <ChatIcon className="w-4 h-4" /> 
        </button>
        
        {/* 絵文字ピッカー */}
        {showPicker && (
          /* ▼▼▼ [修正] ピッカーのUI変更 (折り返し、カスタムボタン) ▼▼▼ */
          <div className="absolute top-7 left-0 z-10 bg-white shadow-lg rounded-lg border border-gray-200 p-2 flex flex-wrap gap-1 w-64">
            {!showCustomInput ? (
              <>
                {/* 標準スタンプ */}
                {EMOJIS.map(emoji => (
                  <button
                    ...
                    className="text-lg p-1.5 rounded-md hover:bg-gray-100 transition-transform active:scale-90"
                  >
                    {emoji}
                  </button>
                ))}
                ...
                <button
                  ...
                  className="text-base p-1.5 rounded-md hover:bg-gray-100 transition-transform active:scale-90 text-gray-500"
                >
                  +
                </button>
              </>
              ...
              <div className="w-full flex gap-1 p-1">
                <Input 
                  ...
                  autoFocus
                />
                <Button size="sm" onClick={handleAddCustomEmoji}>追加</Button>
              </div>
            )}
          </div>
          /* ▲▲▲ 修正 ▲▲▲ */
  ...
  const renderPost = (p: Post, view: TimelineView) => {
    const authorName = p.authorId === 'anonymous' ? '匿名' : getMemberName(p.authorId);
    let recipientName = '';
    ...
    /* ▼▼▼ [修正] 'みんな' (all) ビューの時は From -> To 形式で表示 ▼▼▼ */
    let title = '';
    if (view === 'my') {
      ...
      /* ▼▼▼ [修正] 'From: (差出人) → To: (宛先)' の形式に変更 ▼▼▼ */
      title = `From: ${authorName} → To: ${recipientName}`;
    }
    /* ▲▲▲ 修正 ▲▲▲ */
    ...
    return (
      <div key={p.id} className="py-3 border-b last:border-b-0">
        ...
        <div className="flex items-center justify-between mt-2">
          {/* Reactions */}
          {/* ▼▼▼ [修正] 静的な表示から ReactionBar コンポーネントに変更 ▼▼▼ */}
          <ReactionBar post={p} />
          {/* ▲▲▲ 修正 ▲▲▲ */}
          {/* Reply Status */}
          {view === 'inbox' && !p.replied && (
            <span className="text-xs font-semibold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full">未返信</span>
          )}
        </div>
      </div>
    );
  ...
  /* ▼▼▼ [追加] エラー修正: categoryUI オブジェクトを定義 ▼▼▼ */
  const categoryUI: Record<Category, { tab: string; header: string; desc: string; }> = {
    thanks: { tab: "ありがとう", header: "ありがとう", desc: "ちょっとした感謝をひとことで。" },
    fix: { tab: "ここ直して", header: "ここ直してほしい", desc: "改善してほしい点を具体的に。" },
    help: { tab: "困ってます", header: "困ってます", desc: "詰まり・困りごとを簡潔に。" },
    together: { tab: "コラボ", header: "コラボ", desc: "コラボレーションの提案。" },
    customer: { tab: "お客さまの声", header: "お客さまの声", desc: "印象に残ったお客さまの言葉を要約し、背景や気づきをひと言で。" },
  };
  /* ▲▲▲ 追加 ▲▲▲ */

  const catTheme: Record<Category, { bg: string; text: string; border: string; button: string; active: string; tab: string; }> = {
    thanks: { bg: "bg-rose-50", text: "text-rose-700", border: "border-rose-300", button: "bg-rose-600 text-white hover:bg-rose-700", active: "bg-rose-600 text-white", tab: "bg-white text-rose-700 border border-rose-300 hover:bg-rose-50" },
    fix: { bg: "bg-amber-50", text: "text-amber-700", border: "border-amber-300", button: "bg-amber-500 text-white hover:bg-amber-600", active: "bg-amber-500 text-white", tab: "bg-white text-amber-700 border border-amber-300 hover:bg-amber-50" },
    help: { bg: "bg-sky-50", text: "text-sky-700", border: "border-sky-300", button: "bg-sky-600 text-white hover:bg-sky-700", active: "bg-sky-600 text-white", tab: "bg-white text-sky-700 border border-sky-300 hover:bg-sky-50" },
    together: { bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-300", button: "bg-emerald-600 text-white hover:bg-emerald-700", active: "bg-emerald-600 text-white", tab: "bg-white text-emerald-700 border border-emerald-300 hover:bg-emerald-50" },
    customer: { bg: "bg-purple-50", text: "text-purple-700", border: "border-purple-300", button: "bg-purple-600 text-white hover:bg-purple-700", active: "bg-purple-600 text-white", tab: "bg-white text-purple-700 border border-purple-300 hover:bg-purple-50" },
  };
  const placeholderByCat: Record<Category, string> = {
    thanks: "例：誰に・どの対応が良かった？「○○さん、カウンセリング神対応でした！」",
    fix: "例：課題→原因→提案「テスター切れ→補充動線が遠い→トレーをこの棚へ」",
    help: "例：今の困りごと「色番○○の在庫だけ欠品続き」",
    together: "例：誰と何を？「VMD変更、△△さん一緒に10分だけお願いします」",
    customer: "例：要約＋背景「敏感肌の方から“この化粧水は刺激が少ない”と3名」",
  };
  ...
  const handleSubmit = () => {
    ...
    const newPost: Post = {
      ...
      recipient: recipient.length > 1 ? recipient : recipient[0], // 複数または単一
      text,
      ts: Date.now(),
      reactions: [],
    };
    ...
    // 'all' タイムラインに追加 (全員宛 or 1-on-1)
    if (recipient.includes('zen') || !anonymous) { // 全員宛か、匿名でない1on1は 'all' にも表示
      setAllPublicPosts([newPost, ...allPublicPosts]);
    }
    
    // TODO: 'inbox' への追加 (受信者側での処理)
    ...
    <div className="w-full max-w-6xl mx-auto">
      <div className="text-center mb-6">
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-gray-900 leading-tight">
          <span className="bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent">tsumugi</span> ひとことボード
        </h1>
        <p className="text-sm text-gray-600 mt-1">チームワークを高めるための小さな発信ツール</p>
      </div>
          ...
          {Object.keys(catTheme).map(catKey => {
            ...
            return (
              <VoiceTab key={key} value={key} className={`${category === key ? catTheme[key].active : catTheme[key].tab}`}>
                {categoryUI[key].tab} {/* ▼▼▼ [修正] categoryTitle -> categoryUI[key].tab ▼▼▼ */ }
              </VoiceTab> 
            )
          })}
        </VoiceTabList>
                ...
                <CardHeader className="pb-3">
                  <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                    <div className="text-2xl sm:text-3xl font-extrabold text-gray-900">
                      {categoryUI[key].header} {/* ▼▼▼ [修正] categoryTitle -> categoryUI[key].header ▼▼▼ */ }
                    </div>
                    <div className="text-sm text-gray-600">
                      {categoryUI[key].desc} {/* ▼▼▼ [修正] categoryDesc -> categoryUI[key].desc ▼▼▼ */ }
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="grid gap-4">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-700 font-medium">誰に送りますか？</span>
        ...
        <Tabs value={timelineView} onChange={(v) => setTimelineView(v as TimelineView)}>
          <TabList className="bg-gray-100 rounded-t-lg border-gray-200 sticky top-16 z-10 shadow-inner inset-x-0 px-1 sm:px-4">
            <Tab value="my" className={`flex-1 sm:flex-initial ${timelineView === 'my' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-600 hover:text-gray-800'}`}>
              ...
              <span className="hidden sm:inline">あなた発（{myPosts.length}）</span>
            </Tab>
            <Tab value="inbox" className={`flex-1 sm:flex-initial ${timelineView === 'inbox' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-600 hover:text-gray-800'}`}>
              ...
              <span className="hidden sm:inline">あなた宛（{inboxPosts.length}）</span>
            </Tab>
            <Tab value="all" className={`flex-1 sm:flex-initial ${timelineView === 'all' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-600 hover:text-gray-800'}`}>
              ...
              <span className="hidden sm:inline">みんな（{allPublicPosts.length}）</span>
            </Tab>
          </TabList>
          ...
          <Card className="rounded-t-none">
            <CardContent className="pt-5">
              <TabPanel when="my">
                ...
                {myPosts.length > previewCount && <Button variant="outline" size="sm" className="w-full mt-2" onClick={() => openModal('my')}>すべて見る（{myPosts.length}件）</Button>}
              </TabPanel>
              ...
              <TabPanel when="inbox">
                {inboxPosts.length > 0 ? (
                  ...
                  <p className="text-sm text-gray-500">あなた宛ての投稿はまだありません。</p>
                )}
                {inboxPosts.length > previewCount && <Button variant="outline" size="sm" className="w-full mt-2" onClick={() => openModal('inbox')}>すべて見る（{inboxPosts.length}件）</Button>}
              </TabPanel>
              ...
              <TabPanel when="all">
                ...
                {allPublicPosts.length > previewCount && <Button variant="outline" size="sm" className="w-full mt-2" onClick={() => openModal('all')}>すべて見る（{allPublicPosts.length}件）</Button>}
              </TabPanel>
            </CardContent>
          </Card>
        </Tabs>
        ...
        <div className="max-h-[60vh] overflow-y-auto pr-2">
          {modalPosts.length > 0 ? (
            modalPosts.map(p => renderPost(p, 
              modalTitle.includes('あなた') ? (modalTitle.includes('宛て') ? 'inbox' : 'my') : 'all'
            ))
          ) : (
            <p className="text-sm text-gray-500">まだ投稿がありません。</p>
          )}
...
function KnowledgeBase({ role }: { role: string }){
  ...
  const [managerMsg, setManagerMsg] = useState<Notice[]>([...INITIAL_NOTICES, { id:'n2', title:'【再送】年末調整の対応', body:'未提出者は本日中に必ず対応してください。', created:'2025-10-28 10:00' }, { id:'n3', title:'棚卸しスケジュール', body:'今週末の棚卸し担当は添付ファイル確認。', created:'2025-10-27 15:00' }, { id:'n4', title:'MTGキャンセル', body:'本日の夕礼はキャンセルです。', created:'2025-10-27 09:00' }]);
    ...
    <div key={f.id} className="border rounded-xl p-4 bg-white shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-gray-500">{f.type} • {f.uploaded} • {f.size}</div>
          <div className="mt-3 flex gap-2">
            <Button variant="outline" size="sm">ファイルを開く</Button>
            <Button variant="primary" size="sm" onClick={()=>openAISummary(f)}>
              <SparklesIcon className="w-4 h-4 mr-1.5"/>AI要約を見る
            </Button>
          </div>
        </div>
      </div>
  ...
  if(viewMode === 'all_files'){ 
    return (
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-semibold">すべてのファイル（Box）</h1>
          <Button variant="outline" size="sm" onClick={()=>setViewMode('latest')}>最新に戻る</Button>
        </div>
       
        <Card>
          <CardHeader>
            <div className="text-base font-semibold">ファイル通達（フォルダ別）</div>
            <div className="text-xs text-gray-500">AI要約付きのファイル群です。</div>
          </CardHeader>
          <CardContent>
              ...
              {allFolders.map(folder => (
                <div key={folder.id} className="border rounded-lg p-3 flex items-center gap-3 hover:bg-gray-50 cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500">更新: {folder.updated}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="border rounded-lg overflow-hidden">
              <div className="hidden md:grid grid-cols-12 px-4 py-2 text-xs text-gray-500 bg-gray-50 font-medium">
                ...
                <div className="col-span-2">フォルダ</div>
              ...
              {allFiles.map(f => (
                <div key={f.id} className="grid grid-cols-1 md:grid-cols-12 px-4 py-3 items-center border-t text-sm">
                  ...
                  <div className="col-span-12 md:col-span-2 text-xs text-gray-600 md:text-sm">
                    {allFolders.find(fold => fold.id === f.folderId)?.name || 'ルート'}
                  </div>
                  ...
                  <div className="col-span-12 md:col-span-3 flex items-center gap-2 justify-start md:justify-end mt-2 md:mt-0">
                    <Button variant="outline" size="sm">ファイルを開く</Button>
                    <Button variant="primary" size="sm" onClick={()=>openAISummary(f)}>
                      <SparklesIcon className="w-4 h-4 mr-1"/>AI要約
                    </Button>
                  </div>
                </div>
              ))}
        ...
        <Modal open={aiOpen} title={aiTarget? (('body' in aiTarget) ? aiTarget.title : `${aiTarget.title} のAI要約`) : ''} onClose={()=>setAiOpen(false)}>
          {aiTarget && ('body' in aiTarget ? <div className="whitespace-pre-line">{aiTarget.body}</div> : <div className="space-y-2"><p>（ダミーAI要約）</p><p>1. 重要ポイントは「リピジュアEX」です。</p><p>2. 現場では「敏感肌のお客様」への声かけを徹底してください。</p><p>3. 注意事項は「既存品との併用は非推奨」です。</p></div>)}
        </Modal>
      </div>
    );
  }
  ...
  if(viewMode === 'all_messages'){
    return (
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-semibold">すべての店長メッセージ</h1>
          <Button variant="outline" size="sm" onClick={()=>setViewMode('latest')}>最新に戻る</Button>
        </div>
       
        <Card>
          <CardHeader>
            <div className="text-base font-semibold">店長メッセージ（全件）</div>
            <div className="text-xs text-gray-500">AI要約なしの短文通達です。</div>
          </CardHeader>
          {role==='manager' && (
            <CardContent className="border-b grid gap-2">
              <div className="text-sm font-semibold">新規メッセージ作成</div>
              <Input placeholder="タイトル" value={draftTitle} onChange={e=>setDraftTitle(e.target.value)} />
              <Textarea rows={3} placeholder="本文（短文でOK）" value={draftBody} onChange={e=>setDraftBody(e.target.value)} />
              <div className="text-right"><Button onClick={addMsg} disabled={!draftTitle.trim() || !draftBody.trim()}>メッセージを追加</Button></div>
            </CardContent>
          )}
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <div className="text-base font-semibold">店長メッセージ（最新）</div>
              <div className="text-xs text-gray-500">AI要約なし。クリックでポップアップ表示。</div>
            </div>
            <Button variant="link" size="sm" onClick={()=>setViewMode('all_messages')}>過去のメッセージを見る</Button>
          </div>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        {role==='manager' && (
          <CardContent className="border-t">
            <div className="text-xs text-gray-500 mb-2">新規メッセージ作成</div>
            <div className="grid gap-2">
              <Input placeholder="タイトル" value={draftTitle} onChange={e=>setDraftTitle(e.target.value)} />
              <Textarea rows={3} placeholder="本文（短文でOK）" value={draftBody} onChange={e=>setDraftBody(e.target.value)} />
              <div className="text-right"><Button onClick={addMsg} disabled={!draftTitle.trim() || !draftBody.trim()}>追加</Button></div>
            </div>
          </CardContent>
        )}
      </Card>
      ...
      <div className="flex items-center justify-between mt-4">
        <h1 className="text-base sm:text-lg font-semibold">ファイル通達（最新・重要）</h1>
        <Button variant="outline" size="sm" onClick={()=>setViewMode('all_files')}>すべてのファイルを見る</Button>
      </div>
...
function RelationshipDashboard(){
  ...
  const WeatherIcon = ({ status }: { status: WeatherStatus }) => {
    if (status === 'good') return <CheckCircleIcon className="w-5 h-5 text-green-500"/>; // 晴れ
    if (status === 'cloudy') return <ExclamationCircleIcon className="w-5 h-5 text-amber-500"/>; // くもり
    if (status === 'bad') return <ExclamationCircleIcon className="w-5 h-5 text-rose-500"/>; // 雨
    return null;
  };

  const cards: KpiCard[] = [
    {k:'Flow (流れ)',v:74,sub:'滞留: B店・夜間', status: 'cloudy'},
    {k:'Tone (温度)',v:68,sub:'ネガ比率↑（C店）', status: 'bad'},
    {k:'Rhythm (継続)',v:72,sub:'応答周期の乱れ', status: 'cloudy'},
    {k:'Voice (声)',v:81,sub:'反映率 63%', status: 'good'},
    {k:'Trust (信頼)',v:76,sub:'前週 +2', status: 'good'},
  ];

  const summaryPoints: string[] = [
    "「山口さん」「佐藤さん」の応答遅延（Rhythmの詰まり）が顕著です。",
    "全店的に「改善提案（Voice）」の反映率が低下傾向にあります（平均63%）。",
    "「山口さん」「佐藤さん」に業務負荷集中とネガティブ投稿の兆候が見られます。"
  ];
 
  const risks: Risk[] = [
    {name:'山口', why:'Voice減＋ネガ高＋欠勤2', action:'夜勤後の15分1on1で「最近の困りごと」確認。土曜昼帯の応援1枠追加を検討。'},
    {name:'佐藤', why:'反映待ち7件/14日', action:'「未反映の具体」を棚卸し→今日中に3件だけ着手できる小タスク化。店長が1件代行着手し勢いをつける。'},
    {name:'田中', why:'連続深夜＋投稿0/10日', action:'次シフトで軽作業へローテ。朝礼で称賛1トピックを付与し、Voice再開の合図を作る。'},
  ];

  const bottleneckData: Bottleneck[] = [
    { team: '鈴木', response: 4, resolution: 12, color: 'bg-green-300' },
    { team: '木村', response: 6, resolution: 18, color: 'bg-green-200' },
    { team: '田中', response: 12, resolution: 36, color: 'bg-amber-300' },
    { team: '山口', response: 28, resolution: 72, color: 'bg-rose-400' }, // 詰まり
    { team: '佐藤', response: 20, resolution: 48, color: 'bg-amber-400' }, // 詰まり
  ];
  ...
  return (
      ...
      <Card className="bg-gradient-to-br from-indigo-50 to-purple-50 border-indigo-200 shadow-lg">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <div className="font-semibold text-lg text-gray-900">AIコーチ（今週の優先アクション）</div>
          </div>
          <div className="text-sm text-gray-600">ERMデータと勤怠データから、組織課題と個別アラートを抽出しました。</div>
        </CardHeader>
        <CardContent>
          <div className="mb-4 border-b pb-4">
            <div className="text-sm font-semibold text-gray-800 mb-2">▼ サマリー（全体傾向）</div>
          ...
          <div className="text-sm font-semibold text-gray-800 mb-2">▼ 優先対応アラート（個別）</div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {risks.map((r,i)=> (
              <div key={i} className="border rounded-2xl p-4 bg-white shadow-sm">
                ...
                <div className="text-xs text-gray-500 mb-2 ml-7">兆候：{r.why}</div>
                <div className="text-sm text-gray-800">提案：{r.action}</div>
                <div className="mt-3 flex gap-2 text-xs">
                  <Button size="sm" variant="outline">声かけテンプレ</Button>
                  <Button size="sm" variant="outline">1on1を作成</Button>
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <Card>
        <CardHeader><div className="font-semibold text-sm">Voice反映率（店舗別・週次）</div></CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader><div className="font-semibold text-sm">Trust成長度（全社・月次）</div></CardHeader>
        <CardContent>
      ...
      <Card> 
        <CardHeader><div className="font-semibold text-sm">Rhythm詰まりマップ（スタッフ別 平均応答時間）</div></CardHeader>
        <CardContent>
          <div className="space-y-2">
            <div className="flex text-xs text-gray-500">
              <div className="w-1/3">スタッフ名</div>
              <div className="w-2/3">応答時間（h） [ 0 ... 24 ... 48 ... 72+ ]</div>
            </div>
            ...
            <div className="flex items-center gap-4 text-xs text-gray-500 pt-2">
              <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-green-300"/> 良好 (～12h)</span>
              <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-amber-300"/> 注意 (～24h)</span>
              <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-rose-400"/> 危険 (24h+)</span>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader><div className="font-semibold text-sm">参考：事実データ（連携SaaS）</div></CardHeader>
        <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="border p-3 rounded-lg text-center">
    <div className="text-xs text-gray-500">KOT (勤怠)</div>
    <div className="text-lg font-semibold">遅刻 3名</div>
  </div>
  <div className="border p-3 rounded-lg text-center">
    <div className="text-xs text-gray-500">KOT (勤怠)</div>
    <div className="text-lg font-semibold">平均残業 18.5h</div>
  </div>
  <div className="border p-3 rounded-lg text-center">
    <div className="text-xs text-gray-500">カオナビ (評価)</div>
    <div className="text-lg font-semibold">評価 進捗 64%</div>
  </div>
  <div className="border p-3 rounded-lg text-center">
    <div className="text-xs text-gray-500">カオナビ (スキル)</div>
    <div className="text-lg font-semibold">新商品テスト 82点</div>
  </div>
        </CardContent>
      </Card>
</div>
...
function Integrations(){
  const items: IntegrationItem[] = [
    { k:'KOT', label:'KING OF TIME (勤怠)', status:'linked', sub:'最終同期: 2025-11-01 00:05' },
    { k:'KAO', label:'カオナビ (タレント)', status:'linked', sub:'最終同期: 2025-10-31 23:00' },
    { k:'SHR', label:'SmartHR (労務)', status:'unlinked', sub:'従業員マスタの連携を推奨します。' },
    { k:'KIN', label:'kintone (業務)', status:'linked', sub:'連携DB: kintone-db-123' },
  ];
 ...
 return (
    <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="md:col-span-2 p-4 bg-white rounded-2xl border">
        <h3 className="font-semibold">中立的なERMハブ戦略</h3>
        <p className="text-sm text-gray-600">tsumugiは、既存のHR TechSaaSと連携し、彼らが持てない「関係性データ」を掛け合わせることで価値を生みます。</p>
  </div>
      {items.map(it => (
        <Card key={it.k}>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${chip(it.status)}`}>{it.status==='linked'?'連携済み':'未連携'}</span>
          </div>
          </CardHeader>
          <CardContent>
            <div className="text-sm text-gray-600">{it.sub}</div>
            {it.status === 'unlinked' && <Button size="sm" variant="outline" className="mt-3">連携する</Button>}
          </CardContent>
        </Card>
      ))}
    </div>
